...
try:
	...
	from EAST.geo_map_cython_lib import gen_geo_map
...
def load_annotation_original(p):
	'''
...
def point_dist_to_line_original(p1, p2, p3):
...
def sort_rectangle_new(poly):
	...
	if np.count_nonzero((poly[:, 1] - poly[p_lowest, 1]) < 1e-3) == 2:
		...
		if angle/np.pi * 180 > 45:
			...
			p0_index = (p2_index - 2) % 4
			p3_index = (p2_index + 1) % 4
			return poly[[p0_index, p1_index, p2_index, p3_index]], -(np.pi/2 - angle), ok
		else:
			p3_index = p_lowest
			p0_index = (p3_index + 1) % 4
			p1_index = (p3_index + 2) % 4
			p2_index = (p3_index + 3) % 4
			return poly[[p0_index, p1_index, p2_index, p3_index]], angle, ok
...
def get_text_file(image_file):
	...
	txt_file = txt_file.replace(txt_file_name,  txt_file_name)
	return txt_file
...
def count_samples(FLAGS):
	if sys.version_info >= (3, 0):
		return len([f for f in next(os.walk(FLAGS.training_data_path))[2] if f[-4:] == ".jpg" or f[-4:] == ".png"])
	else:
		return len([f for f in os.walk(FLAGS.training_data_path).next()[2] if f[-4:] == ".jpg" or f[-4:] == ".png"])
...
def load_data(FLAGS, is_train=False):
	...
	#pool = Pool()
	if sys.version_info >= (3, 0):
		#print(pool.map_async(load_data_process, zip(image_files, itertools.repeat(FLAGS), itertools.repeat(is_train))))
	...
	#print(loaded_data)
	#loaded_data = [load_data_process([image_files[i], FLAGS, is_train]) for i in range(len(image_files))]
	pool.close()
	pool.join()
